...
const LoginPage: React.FC = () => {
  ...
  const [phase, setPhase] = React.useState<
    "idle" | "toBlack" | "toGold" | "adminLogin" | "managerLogin"
  >("idle");
  const [hoverSide, setHoverSide] = React.useState<"none" | "left" | "right">(
    "none"
  );
  ...
  const [showVerificationContainer, setShowVerificationContainer] =
    useState(false);
  const [showNewPassword, setShowNewPassword] = useState(false);
  const [showNewPasswordContainer, setShowNewPasswordContainer] =
    useState(false);
  ...
  const [showManagerForgotContainer, setShowManagerForgotContainer] =
    useState(false);
              ...
              <div className="grid grid-cols-3 gap-4 max-w-xs mx-auto">
                {[1, 2, 3, 4, 5, 6, 7, 8, 9].map((num) => (
                  <button
                    ...
                    onClick={() => {
                      const emptyIndex = pinCode.findIndex(
                        (digit) => digit === ""
                      );
                      if (emptyIndex !== -1) {
