...
export async function GET(req: Request) {
  ...
  if (searchParams.get("parentId")) queryParams.append("parentId", searchParams.get("parentId")!);
  ...
  const url = `${getRemoteBase()}/t/menu/categories${queryString ? `?${queryString}` : ''}`;

  const headers = buildTenantHeaders(req, true);

  console.log("🌐 GET /api/menu/categories - Proxying to:", url);
...
export async function POST(req: Request) {
  ...
  const url = `${getRemoteBase()}/t/menu/categories`;

  const headers = buildTenantHeaders(req, true);

  console.log("🌐 POST /api/menu/categories - Creating category");
  console.log("📋 Headers:", headers);
