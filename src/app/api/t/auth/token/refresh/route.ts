...
export async function POST(req: Request) {
  try {
    ...
    const url = `${REMOTE_BASE}/t/auth/token/refresh`;
    ...
    return NextResponse.json({ success: false, message: err?.message || "Proxy POST /t/auth/token/refresh failed" }, { status: 500 });
  }
}